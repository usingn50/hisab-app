...
- [x] App icon asset generation (png/adaptive icon files; color direction decided: gold/blue, simplified mark, no text)
...
- [x] Business info edit UI (settings_screen currently shows businessName read-only; users table has businessName/businessType/city with placeholder defaults "متجري"/"عام"/"—" set at ensureExists() — needs an edit form + userRepositoryProvider.updateBusinessInfo() call)
